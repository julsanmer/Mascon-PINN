...
class BSKSimulation(SimulationBaseClass.SimBaseClass):
    ...
    class BSKDynamicModels:
        ...
        def initialize_dynamics(self, asteroid, sc, flag_sun):
            # Fill variables for each dynamics module (order matters)
            self.set_ephemeris()
            self.set_gravity(asteroid)
            if flag_sun:
        ...
        def set_gravity(self, asteroid):
            ...
            for gravity in asteroid.gravity:
                # Mascon gravity
                if gravity.name == 'mascon':
                    mu += np.sum(gravity.mu_M)
                    mascon = self.gravFactory.createCustomGravObject('mascon', np.sum(gravity.mu_M))
                    self.asteroid.append(mascon)
                    self.asteroid[idx].useMasconGravityModel()
                    self.asteroid[idx].mascon.muMascon = gravity.mu_M.tolist()
                    self.asteroid[idx].mascon.xyzMascon = gravity.xyz_M.tolist()
                    self.asteroid[idx].planetBodyInMsg.subscribeTo(self.gravBodyEphem.planetOutMsgs[0])
                    idx += 1
                # Spherical harmonics gravity
                    ...
                    self.asteroid[idx].useSphericalHarmParams = True
                    self.asteroid[idx].spherHarm.cBar = gravityEffector.MultiArray(gravity.C)
                    self.asteroid[idx].spherHarm.sBar = gravityEffector.MultiArray(gravity.S)
                    self.asteroid[idx].mu = gravity.mu
                    self.asteroid[idx].spherHarm.maxDeg = gravity.deg
                    self.asteroid[idx].radEquator = gravity.rE
                    self.asteroid[idx].planetBodyInMsg.subscribeTo(self.gravBodyEphem.planetOutMsgs[0])
                    idx += 1
                # PINN gravity
                    ...
                    self.asteroid[idx].usePINN2GravityModel()
                    self.asteroid[idx].pinn.PINNPath = gravity.file_torch
                    self.asteroid[idx].planetBodyInMsg.subscribeTo(self.gravBodyEphem.planetOutMsgs[0])
                    idx += 1
                # Polyhedron gravity
                    ...
                    self.asteroid[idx].usePolyhedralGravityModel(gravity.file)
                    self.asteroid[idx].planetBodyInMsg.subscribeTo(self.gravBodyEphem.planetOutMsgs[0])
                    idx += 1
                else:
