...
void CameraNav2Msg_C_addAuthor(CameraNav2Msg_C *coownerMsg, CameraNav2Msg_C *targetMsg) {
...
void CameraNav2Msg_C_init(CameraNav2Msg_C *owner) {
    ...
    if (owner->payloadPointer == 0) {
        CameraNav2Msg_C_addAuthor(owner, owner);
    }
};
...
CameraNav2MsgPayload CameraNav2Msg_C_zeroMsgPayload() {
    CameraNav2MsgPayload zeroMsg;
    memset(&zeroMsg, 0x0, sizeof(CameraNav2MsgPayload));
    return zeroMsg;
};
...
CameraNav2MsgPayload CameraNav2Msg_C_read(CameraNav2Msg_C *source) {
    if (!source->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are trying to read an un-written message of type CameraNav2Msg.");
    }
...
int CameraNav2Msg_C_isWritten(CameraNav2Msg_C *data) {
    ...
    BSK_PRINT(MSG_ERROR,"In C input msg, you are checking if an unconnected msg of type CameraNav2Msg is written.");
    return 0;
};
...
uint64_t CameraNav2Msg_C_timeWritten(CameraNav2Msg_C *data) {
    ...
    BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting the write time of an unconnected msg of type CameraNav2Msg.");
    return 0;
};
...
int64_t CameraNav2Msg_C_moduleID(CameraNav2Msg_C *data) {
    if (!data->header.isLinked) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unconnected msg of type CameraNav2Msg.");
        return 0;
    }
    if (!data->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unwritten msg of type CameraNav2Msg.");
        return 0;
    }
...
void CameraNav2Msg_cpp_subscribe(CameraNav2Msg_C *subscriber, void* source){
    Message<CameraNav2MsgPayload>* source_t = (Message<CameraNav2MsgPayload>*) source;
    MsgHeader *msgPtr;
