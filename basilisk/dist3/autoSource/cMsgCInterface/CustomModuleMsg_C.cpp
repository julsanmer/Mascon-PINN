...
void CustomModuleMsg_C_addAuthor(CustomModuleMsg_C *coownerMsg, CustomModuleMsg_C *targetMsg) {
...
void CustomModuleMsg_C_init(CustomModuleMsg_C *owner) {
    ...
    if (owner->payloadPointer == 0) {
        CustomModuleMsg_C_addAuthor(owner, owner);
    }
};
...
CustomModuleMsgPayload CustomModuleMsg_C_zeroMsgPayload() {
    CustomModuleMsgPayload zeroMsg;
    memset(&zeroMsg, 0x0, sizeof(CustomModuleMsgPayload));
    return zeroMsg;
};
...
CustomModuleMsgPayload CustomModuleMsg_C_read(CustomModuleMsg_C *source) {
    if (!source->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are trying to read an un-written message of type CustomModuleMsg.");
    }
...
int CustomModuleMsg_C_isWritten(CustomModuleMsg_C *data) {
    ...
    BSK_PRINT(MSG_ERROR,"In C input msg, you are checking if an unconnected msg of type CustomModuleMsg is written.");
    return 0;
};
...
uint64_t CustomModuleMsg_C_timeWritten(CustomModuleMsg_C *data) {
    ...
    BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting the write time of an unconnected msg of type CustomModuleMsg.");
    return 0;
};
...
int64_t CustomModuleMsg_C_moduleID(CustomModuleMsg_C *data) {
    if (!data->header.isLinked) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unconnected msg of type CustomModuleMsg.");
        return 0;
    }
    if (!data->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unwritten msg of type CustomModuleMsg.");
        return 0;
    }
...
void CustomModuleMsg_cpp_subscribe(CustomModuleMsg_C *subscriber, void* source){
    Message<CustomModuleMsgPayload>* source_t = (Message<CustomModuleMsgPayload>*) source;
    MsgHeader *msgPtr;
