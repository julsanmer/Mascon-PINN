...
#include "SmallBodyNavIntMsg_C.h"
#include "architecture/messaging/messaging.h"
#include "architecture/utilities/bsk_Print.h"
#include<string.h>

//! C interface to subscribe to a message
void SmallBodyNavIntMsg_C_subscribe(SmallBodyNavIntMsg_C *subscriber, SmallBodyNavIntMsg_C *source) {
...
int8_t SmallBodyNavIntMsg_C_isSubscribedTo(SmallBodyNavIntMsg_C *subscriber, SmallBodyNavIntMsg_C *source) {
...
void SmallBodyNavIntMsg_C_addAuthor(SmallBodyNavIntMsg_C *coownerMsg, SmallBodyNavIntMsg_C *targetMsg) {
...
void SmallBodyNavIntMsg_C_init(SmallBodyNavIntMsg_C *owner) {
    //! check if the msg pointer is not assigned already.  If not, then connect message to itself.
    if (owner->payloadPointer == 0) {
        SmallBodyNavIntMsg_C_addAuthor(owner, owner);
    }
};

//! C interface to write to a message
void SmallBodyNavIntMsg_C_write(SmallBodyNavIntMsgPayload *data, SmallBodyNavIntMsg_C *destination, int64_t moduleID, uint64_t callTime) {
...
SmallBodyNavIntMsgPayload SmallBodyNavIntMsg_C_zeroMsgPayload() {
    SmallBodyNavIntMsgPayload zeroMsg;
    memset(&zeroMsg, 0x0, sizeof(SmallBodyNavIntMsgPayload));
    return zeroMsg;
};


//! C interface to read to a message
SmallBodyNavIntMsgPayload SmallBodyNavIntMsg_C_read(SmallBodyNavIntMsg_C *source) {
    if (!source->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are trying to read an un-written message of type SmallBodyNavIntMsg.");
    }
...
int SmallBodyNavIntMsg_C_isLinked(SmallBodyNavIntMsg_C *data) {
    return (int) data->header.isLinked;
};

//! C interface to see if this message container ever been written to
int SmallBodyNavIntMsg_C_isWritten(SmallBodyNavIntMsg_C *data) {
    if (data->header.isLinked) {
        return (int) data->headerPointer->isWritten;
    }
    BSK_PRINT(MSG_ERROR,"In C input msg, you are checking if an unconnected msg of type SmallBodyNavIntMsg is written.");
    return 0;
};

//! C interface to see if this message container ever been written to
uint64_t SmallBodyNavIntMsg_C_timeWritten(SmallBodyNavIntMsg_C *data) {
    if (data->header.isLinked) {
        return data->headerPointer->timeWritten;
    }
    BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting the write time of an unconnected msg of type SmallBodyNavIntMsg.");
    return 0;
};

//! C interface to get the moduleID of who wrote the message
int64_t SmallBodyNavIntMsg_C_moduleID(SmallBodyNavIntMsg_C *data) {
    if (!data->header.isLinked) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unconnected msg of type SmallBodyNavIntMsg.");
        return 0;
    }
    if (!data->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unwritten msg of type SmallBodyNavIntMsg.");
        return 0;
    }
...
//! method description
void SmallBodyNavIntMsg_cpp_subscribe(SmallBodyNavIntMsg_C *subscriber, void* source){
    Message<SmallBodyNavIntMsgPayload>* source_t = (Message<SmallBodyNavIntMsgPayload>*) source;
    MsgHeader *msgPtr;
...
int8_t SmallBodyNavIntMsg_cpp_isSubscribedTo(SmallBodyNavIntMsg_C *subscriber, void* source) {
