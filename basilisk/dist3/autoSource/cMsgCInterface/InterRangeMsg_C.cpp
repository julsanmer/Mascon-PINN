...
#include "InterRangeMsg_C.h"
#include "architecture/messaging/messaging.h"
#include "architecture/utilities/bsk_Print.h"
#include<string.h>

//! C interface to subscribe to a message
void InterRangeMsg_C_subscribe(InterRangeMsg_C *subscriber, InterRangeMsg_C *source) {
...
int8_t InterRangeMsg_C_isSubscribedTo(InterRangeMsg_C *subscriber, InterRangeMsg_C *source) {
...
void InterRangeMsg_C_addAuthor(InterRangeMsg_C *coownerMsg, InterRangeMsg_C *targetMsg) {
...
void InterRangeMsg_C_init(InterRangeMsg_C *owner) {
    //! check if the msg pointer is not assigned already.  If not, then connect message to itself.
    if (owner->payloadPointer == 0) {
        InterRangeMsg_C_addAuthor(owner, owner);
    }
};

//! C interface to write to a message
void InterRangeMsg_C_write(InterRangeMsgPayload *data, InterRangeMsg_C *destination, int64_t moduleID, uint64_t callTime) {
...
InterRangeMsgPayload InterRangeMsg_C_zeroMsgPayload() {
    InterRangeMsgPayload zeroMsg;
    memset(&zeroMsg, 0x0, sizeof(InterRangeMsgPayload));
    return zeroMsg;
};


//! C interface to read to a message
InterRangeMsgPayload InterRangeMsg_C_read(InterRangeMsg_C *source) {
    if (!source->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are trying to read an un-written message of type InterRangeMsg.");
    }
...
int InterRangeMsg_C_isLinked(InterRangeMsg_C *data) {
    return (int) data->header.isLinked;
};

//! C interface to see if this message container ever been written to
int InterRangeMsg_C_isWritten(InterRangeMsg_C *data) {
    if (data->header.isLinked) {
        return (int) data->headerPointer->isWritten;
    }
    BSK_PRINT(MSG_ERROR,"In C input msg, you are checking if an unconnected msg of type InterRangeMsg is written.");
    return 0;
};

//! C interface to see if this message container ever been written to
uint64_t InterRangeMsg_C_timeWritten(InterRangeMsg_C *data) {
    if (data->header.isLinked) {
        return data->headerPointer->timeWritten;
    }
    BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting the write time of an unconnected msg of type InterRangeMsg.");
    return 0;
};

//! C interface to get the moduleID of who wrote the message
int64_t InterRangeMsg_C_moduleID(InterRangeMsg_C *data) {
    if (!data->header.isLinked) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unconnected msg of type InterRangeMsg.");
        return 0;
    }
    if (!data->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unwritten msg of type InterRangeMsg.");
        return 0;
    }
...
//! method description
void InterRangeMsg_cpp_subscribe(InterRangeMsg_C *subscriber, void* source){
    Message<InterRangeMsgPayload>* source_t = (Message<InterRangeMsgPayload>*) source;
    MsgHeader *msgPtr;
...
int8_t InterRangeMsg_cpp_isSubscribedTo(InterRangeMsg_C *subscriber, void* source) {
