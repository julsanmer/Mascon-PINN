...
#include "SmallBodyNav4Msg_C.h"
#include "architecture/messaging/messaging.h"
#include "architecture/utilities/bsk_Print.h"
#include<string.h>

//! C interface to subscribe to a message
void SmallBodyNav4Msg_C_subscribe(SmallBodyNav4Msg_C *subscriber, SmallBodyNav4Msg_C *source) {
...
int8_t SmallBodyNav4Msg_C_isSubscribedTo(SmallBodyNav4Msg_C *subscriber, SmallBodyNav4Msg_C *source) {
...
void SmallBodyNav4Msg_C_addAuthor(SmallBodyNav4Msg_C *coownerMsg, SmallBodyNav4Msg_C *targetMsg) {
...
void SmallBodyNav4Msg_C_init(SmallBodyNav4Msg_C *owner) {
    //! check if the msg pointer is not assigned already.  If not, then connect message to itself.
    if (owner->payloadPointer == 0) {
        SmallBodyNav4Msg_C_addAuthor(owner, owner);
    }
};

//! C interface to write to a message
void SmallBodyNav4Msg_C_write(SmallBodyNav4MsgPayload *data, SmallBodyNav4Msg_C *destination, int64_t moduleID, uint64_t callTime) {
...
SmallBodyNav4MsgPayload SmallBodyNav4Msg_C_zeroMsgPayload() {
    SmallBodyNav4MsgPayload zeroMsg;
    memset(&zeroMsg, 0x0, sizeof(SmallBodyNav4MsgPayload));
    return zeroMsg;
};


//! C interface to read to a message
SmallBodyNav4MsgPayload SmallBodyNav4Msg_C_read(SmallBodyNav4Msg_C *source) {
    if (!source->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are trying to read an un-written message of type SmallBodyNav4Msg.");
    }
...
int SmallBodyNav4Msg_C_isLinked(SmallBodyNav4Msg_C *data) {
    return (int) data->header.isLinked;
};

//! C interface to see if this message container ever been written to
int SmallBodyNav4Msg_C_isWritten(SmallBodyNav4Msg_C *data) {
    if (data->header.isLinked) {
        return (int) data->headerPointer->isWritten;
    }
    BSK_PRINT(MSG_ERROR,"In C input msg, you are checking if an unconnected msg of type SmallBodyNav4Msg is written.");
    return 0;
};

//! C interface to see if this message container ever been written to
uint64_t SmallBodyNav4Msg_C_timeWritten(SmallBodyNav4Msg_C *data) {
    if (data->header.isLinked) {
        return data->headerPointer->timeWritten;
    }
    BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting the write time of an unconnected msg of type SmallBodyNav4Msg.");
    return 0;
};

//! C interface to get the moduleID of who wrote the message
int64_t SmallBodyNav4Msg_C_moduleID(SmallBodyNav4Msg_C *data) {
    if (!data->header.isLinked) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unconnected msg of type SmallBodyNav4Msg.");
        return 0;
    }
    if (!data->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unwritten msg of type SmallBodyNav4Msg.");
        return 0;
    }
...
//! method description
void SmallBodyNav4Msg_cpp_subscribe(SmallBodyNav4Msg_C *subscriber, void* source){
    Message<SmallBodyNav4MsgPayload>* source_t = (Message<SmallBodyNav4MsgPayload>*) source;
    MsgHeader *msgPtr;
...
int8_t SmallBodyNav4Msg_cpp_isSubscribedTo(SmallBodyNav4Msg_C *subscriber, void* source) {
