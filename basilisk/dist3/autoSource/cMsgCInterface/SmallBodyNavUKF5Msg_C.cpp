...
#include "SmallBodyNavUKF5Msg_C.h"
#include "architecture/messaging/messaging.h"
#include "architecture/utilities/bsk_Print.h"
#include<string.h>

//! C interface to subscribe to a message
void SmallBodyNavUKF5Msg_C_subscribe(SmallBodyNavUKF5Msg_C *subscriber, SmallBodyNavUKF5Msg_C *source) {
...
int8_t SmallBodyNavUKF5Msg_C_isSubscribedTo(SmallBodyNavUKF5Msg_C *subscriber, SmallBodyNavUKF5Msg_C *source) {
...
void SmallBodyNavUKF5Msg_C_addAuthor(SmallBodyNavUKF5Msg_C *coownerMsg, SmallBodyNavUKF5Msg_C *targetMsg) {
...
void SmallBodyNavUKF5Msg_C_init(SmallBodyNavUKF5Msg_C *owner) {
    //! check if the msg pointer is not assigned already.  If not, then connect message to itself.
    if (owner->payloadPointer == 0) {
        SmallBodyNavUKF5Msg_C_addAuthor(owner, owner);
    }
};

//! C interface to write to a message
void SmallBodyNavUKF5Msg_C_write(SmallBodyNavUKF5MsgPayload *data, SmallBodyNavUKF5Msg_C *destination, int64_t moduleID, uint64_t callTime) {
...
SmallBodyNavUKF5MsgPayload SmallBodyNavUKF5Msg_C_zeroMsgPayload() {
    SmallBodyNavUKF5MsgPayload zeroMsg;
    memset(&zeroMsg, 0x0, sizeof(SmallBodyNavUKF5MsgPayload));
    return zeroMsg;
};


//! C interface to read to a message
SmallBodyNavUKF5MsgPayload SmallBodyNavUKF5Msg_C_read(SmallBodyNavUKF5Msg_C *source) {
    if (!source->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are trying to read an un-written message of type SmallBodyNavUKF5Msg.");
    }
...
int SmallBodyNavUKF5Msg_C_isLinked(SmallBodyNavUKF5Msg_C *data) {
    return (int) data->header.isLinked;
};

//! C interface to see if this message container ever been written to
int SmallBodyNavUKF5Msg_C_isWritten(SmallBodyNavUKF5Msg_C *data) {
    if (data->header.isLinked) {
        return (int) data->headerPointer->isWritten;
    }
    BSK_PRINT(MSG_ERROR,"In C input msg, you are checking if an unconnected msg of type SmallBodyNavUKF5Msg is written.");
    return 0;
};

//! C interface to see if this message container ever been written to
uint64_t SmallBodyNavUKF5Msg_C_timeWritten(SmallBodyNavUKF5Msg_C *data) {
    if (data->header.isLinked) {
        return data->headerPointer->timeWritten;
    }
    BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting the write time of an unconnected msg of type SmallBodyNavUKF5Msg.");
    return 0;
};

//! C interface to get the moduleID of who wrote the message
int64_t SmallBodyNavUKF5Msg_C_moduleID(SmallBodyNavUKF5Msg_C *data) {
    if (!data->header.isLinked) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unconnected msg of type SmallBodyNavUKF5Msg.");
        return 0;
    }
    if (!data->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unwritten msg of type SmallBodyNavUKF5Msg.");
        return 0;
    }
...
//! method description
void SmallBodyNavUKF5Msg_cpp_subscribe(SmallBodyNavUKF5Msg_C *subscriber, void* source){
    Message<SmallBodyNavUKF5MsgPayload>* source_t = (Message<SmallBodyNavUKF5MsgPayload>*) source;
    MsgHeader *msgPtr;
...
int8_t SmallBodyNavUKF5Msg_cpp_isSubscribedTo(SmallBodyNavUKF5Msg_C *subscriber, void* source) {
