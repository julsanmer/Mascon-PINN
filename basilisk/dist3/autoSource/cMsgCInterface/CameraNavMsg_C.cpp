...
#include "CameraNavMsg_C.h"
#include "architecture/messaging/messaging.h"
#include "architecture/utilities/bsk_Print.h"
#include<string.h>

//! C interface to subscribe to a message
void CameraNavMsg_C_subscribe(CameraNavMsg_C *subscriber, CameraNavMsg_C *source) {
...
int8_t CameraNavMsg_C_isSubscribedTo(CameraNavMsg_C *subscriber, CameraNavMsg_C *source) {
...
void CameraNavMsg_C_addAuthor(CameraNavMsg_C *coownerMsg, CameraNavMsg_C *targetMsg) {
...
void CameraNavMsg_C_init(CameraNavMsg_C *owner) {
    //! check if the msg pointer is not assigned already.  If not, then connect message to itself.
    if (owner->payloadPointer == 0) {
        CameraNavMsg_C_addAuthor(owner, owner);
    }
};

//! C interface to write to a message
void CameraNavMsg_C_write(CameraNavMsgPayload *data, CameraNavMsg_C *destination, int64_t moduleID, uint64_t callTime) {
...
CameraNavMsgPayload CameraNavMsg_C_zeroMsgPayload() {
    CameraNavMsgPayload zeroMsg;
    memset(&zeroMsg, 0x0, sizeof(CameraNavMsgPayload));
    return zeroMsg;
};


//! C interface to read to a message
CameraNavMsgPayload CameraNavMsg_C_read(CameraNavMsg_C *source) {
    if (!source->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are trying to read an un-written message of type CameraNavMsg.");
    }
...
int CameraNavMsg_C_isLinked(CameraNavMsg_C *data) {
    return (int) data->header.isLinked;
};

//! C interface to see if this message container ever been written to
int CameraNavMsg_C_isWritten(CameraNavMsg_C *data) {
    if (data->header.isLinked) {
        return (int) data->headerPointer->isWritten;
    }
    BSK_PRINT(MSG_ERROR,"In C input msg, you are checking if an unconnected msg of type CameraNavMsg is written.");
    return 0;
};

//! C interface to see if this message container ever been written to
uint64_t CameraNavMsg_C_timeWritten(CameraNavMsg_C *data) {
    if (data->header.isLinked) {
        return data->headerPointer->timeWritten;
    }
    BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting the write time of an unconnected msg of type CameraNavMsg.");
    return 0;
};

//! C interface to get the moduleID of who wrote the message
int64_t CameraNavMsg_C_moduleID(CameraNavMsg_C *data) {
    if (!data->header.isLinked) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unconnected msg of type CameraNavMsg.");
        return 0;
    }
    if (!data->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unwritten msg of type CameraNavMsg.");
        return 0;
    }
...
//! method description
void CameraNavMsg_cpp_subscribe(CameraNavMsg_C *subscriber, void* source){
    Message<CameraNavMsgPayload>* source_t = (Message<CameraNavMsgPayload>*) source;
    MsgHeader *msgPtr;
...
int8_t CameraNavMsg_cpp_isSubscribedTo(CameraNavMsg_C *subscriber, void* source) {
