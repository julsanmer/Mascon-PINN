...
#include "SmallBodyNavHOUMsg_C.h"
...
void SmallBodyNavHOUMsg_C_addAuthor(SmallBodyNavHOUMsg_C *coownerMsg, SmallBodyNavHOUMsg_C *targetMsg) {
...
void SmallBodyNavHOUMsg_C_init(SmallBodyNavHOUMsg_C *owner) {
    ...
    if (owner->payloadPointer == 0) {
        SmallBodyNavHOUMsg_C_addAuthor(owner, owner);
    }
};
...
SmallBodyNavHOUMsgPayload SmallBodyNavHOUMsg_C_zeroMsgPayload() {
    SmallBodyNavHOUMsgPayload zeroMsg;
    memset(&zeroMsg, 0x0, sizeof(SmallBodyNavHOUMsgPayload));
    return zeroMsg;
};
...
SmallBodyNavHOUMsgPayload SmallBodyNavHOUMsg_C_read(SmallBodyNavHOUMsg_C *source) {
    if (!source->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are trying to read an un-written message of type SmallBodyNavHOUMsg.");
    }
...
int SmallBodyNavHOUMsg_C_isWritten(SmallBodyNavHOUMsg_C *data) {
    ...
    BSK_PRINT(MSG_ERROR,"In C input msg, you are checking if an unconnected msg of type SmallBodyNavHOUMsg is written.");
    return 0;
};
...
uint64_t SmallBodyNavHOUMsg_C_timeWritten(SmallBodyNavHOUMsg_C *data) {
    ...
    BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting the write time of an unconnected msg of type SmallBodyNavHOUMsg.");
    return 0;
};
...
int64_t SmallBodyNavHOUMsg_C_moduleID(SmallBodyNavHOUMsg_C *data) {
    if (!data->header.isLinked) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unconnected msg of type SmallBodyNavHOUMsg.");
        return 0;
    }
    if (!data->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unwritten msg of type SmallBodyNavHOUMsg.");
        return 0;
    }
...
void SmallBodyNavHOUMsg_cpp_subscribe(SmallBodyNavHOUMsg_C *subscriber, void* source){
    Message<SmallBodyNavHOUMsgPayload>* source_t = (Message<SmallBodyNavHOUMsgPayload>*) source;
    MsgHeader *msgPtr;
