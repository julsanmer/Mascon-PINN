...
void SmallBodyNav2Msg_C_addAuthor(SmallBodyNav2Msg_C *coownerMsg, SmallBodyNav2Msg_C *targetMsg) {
...
void SmallBodyNav2Msg_C_init(SmallBodyNav2Msg_C *owner) {
    ...
    if (owner->payloadPointer == 0) {
        SmallBodyNav2Msg_C_addAuthor(owner, owner);
    }
};
...
SmallBodyNav2MsgPayload SmallBodyNav2Msg_C_zeroMsgPayload() {
    SmallBodyNav2MsgPayload zeroMsg;
    memset(&zeroMsg, 0x0, sizeof(SmallBodyNav2MsgPayload));
    return zeroMsg;
};
...
SmallBodyNav2MsgPayload SmallBodyNav2Msg_C_read(SmallBodyNav2Msg_C *source) {
    if (!source->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are trying to read an un-written message of type SmallBodyNav2Msg.");
    }
...
int SmallBodyNav2Msg_C_isWritten(SmallBodyNav2Msg_C *data) {
    ...
    BSK_PRINT(MSG_ERROR,"In C input msg, you are checking if an unconnected msg of type SmallBodyNav2Msg is written.");
    return 0;
};
...
uint64_t SmallBodyNav2Msg_C_timeWritten(SmallBodyNav2Msg_C *data) {
    ...
    BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting the write time of an unconnected msg of type SmallBodyNav2Msg.");
    return 0;
};
...
int64_t SmallBodyNav2Msg_C_moduleID(SmallBodyNav2Msg_C *data) {
    if (!data->header.isLinked) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unconnected msg of type SmallBodyNav2Msg.");
        return 0;
    }
    if (!data->headerPointer->isWritten) {
        BSK_PRINT(MSG_ERROR,"In C input msg, you are requesting moduleID of an unwritten msg of type SmallBodyNav2Msg.");
        return 0;
    }
...
void SmallBodyNav2Msg_cpp_subscribe(SmallBodyNav2Msg_C *subscriber, void* source){
    Message<SmallBodyNav2MsgPayload>* source_t = (Message<SmallBodyNav2MsgPayload>*) source;
    MsgHeader *msgPtr;
