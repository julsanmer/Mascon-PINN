...
def plot_orb(parameters, outputs, frame='inertial'):
    ...
    if frame == 'inertial':
        pos = outputs.groundtruth.pos_BP_N1
    elif frame == 'asteroid':
        pos = outputs.groundtruth.pos_BP_P
...
def plot_gravity3D(outputs, parameters):
    h_3D = outputs.groundtruth.hXYZ_3D
    aErr_3D = outputs.results.aErrXYZ_3D
    a0Err_3D = outputs.groundtruth.a0ErrXYZ_3D
    h = outputs.groundtruth.h_BP
